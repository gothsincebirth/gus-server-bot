...
class rules(commands.Cog):
    ...
    async def rules(self, ctx):
        ...
        if ctx.author.id != 420584160495796225:
            ...
            color=0xFFFFFF) 
        # footer
        RulesEmbed.set_footer(text="Staff members have the right to punish any users even if the offense is not listed on the rules")
        # sends the embed 
        await ctx.send(embed=RulesEmbed)
        # creates second embed
        RulesEmbed2 = discord.Embed(
            # embed color
            color=0xFFFFFF
        )
        # second embed image
        RulesEmbed2.set_image(url="https://media.discordapp.net/attachments/923546020049219625/975161751739990036/aesthetic-anime-gif-12-737423972.gif?width=384&height=213")
        # sends second embed
        await ctx.send(embed=RulesEmbed2)
